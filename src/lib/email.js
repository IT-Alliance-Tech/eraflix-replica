...
export async function sendOTPEmail(email, otp, adminName = 'Admin') {
  const mailOptions = {
    ...
    to: email,
    subject: '🔐 DemoBooking Admin - Password Reset OTP',
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
        <div style="background: linear-gradient(45deg, #A855F7, #60A5FA); padding: 20px; border-radius: 10px; text-align: center; margin-bottom: 20px;">
          <h1 style="color: white; margin: 0;">DemoBooking</h1>
...
export async function sendWelcomeEmail(email, username, tempPassword) {
  const mailOptions = {
    from: process.env.EMAIL_USER,
    to: email,
    subject: '🎉 Welcome to DemoBooking Admin',
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
        <div style="background: linear-gradient(45deg, #A855F7, #60A5FA); padding: 20px; border-radius: 10px; text-align: center; margin-bottom: 20px;">
          <h1 style="color: white; margin: 0;">DemoBooking</h1>
...
export async function sendBookingCancellationEmail(customerInfo, booking, reason, refundAmount = 0) {
  try {
    const mailOptions = {
      ...
      subject: `🚫 Booking Cancelled - ${booking.bookingId} | DemoBooking`,
      html: `
        <!DOCTYPE html>
        <html>
        <head>
            ...
            <div class="container">
                ...
                <div class="content">
                    ...
                    <div class="booking-details">
                        ...
                        <p><strong>Space:</strong> ${booking.screen?.name}</p>
                    ...
                    📞 Phone: +1 555 010 9999<br>
                    ✉️ Email: ${process.env.EMAIL_USER}</p>
                    
                    <p>Thank you for choosing DemoBooking.</p>
                </div>
                
                <div class="footer">
                    <p>© ${new Date().getFullYear()} DemoBooking. All rights reserved.</p>
                </div>
            </div>
        </body>
        </html>
...
export async function sendBookingUpdateEmail(customerInfo, booking, updates) {
  try {
    const mailOptions = {
      from: process.env.EMAIL_USER,
      to: customerInfo.email,
      subject: `✅ Booking Updated - ${booking.bookingId} | DemoBooking`,
      html: `
        <!DOCTYPE html>
        <html>
        <head>
...
//   console.log(customerInfo, "custttttttt", booking, "emaillllll");
  ...
  try {
    ...
    const mailOptions = {
      from: process.env.EMAIL_USER,
      to: customerInfo.email,
      subject: `🎉 Booking Confirmed - ${booking.bookingId} | DemoBooking`,
      html: `
        <!DOCTYPE html>
        <html>
        <head>
        ...
        <body>
            <div class="container">
                <div class="header">
                    <h1>🎉 Booking Confirmed!</h1>
                    <p>Your booking experience awaits</p>
                </div>
                ...
                <div class="content">
                    ...
                    <div class="highlight">
                        <p><strong>🎊 Great news! Your booking has been successfully confirmed.</strong></p>
                        <p>Get ready for an amazing experience at DemoBooking!</p>
                    </div>
                    ...
                    <div class="booking-details">
                        <h3>🎬 Your Booking Details:</h3>
                        <p><strong>Booking ID:</strong> ${booking.bookingId}</p>
                        <p><strong>Space:</strong> ${booking.screen?.name || 'Premium Space'}</p>
                        <p><strong>Location:</strong> ${booking.location?.name || 'DemoBooking'}</p>
  ...
  booking.pricing.servicesAmount.toLocaleString()
                    ...
                    <div class="important">
                        ...
                        <p><strong>💰 Payment Methods:</strong> We accept Cash, UPI, Debit/Credit Cards</p>
                        <p><strong>🆔 ID Proof:</strong> Please bring a valid government ID for verification</p>
                        <p><strong>📱 Contact:</strong> Call us at +1 555 010 9999 for any assistance</p>
                    </div>
                </div>
                
                <div class="footer">
                    <p>© ${new Date().getFullYear()} DemoBooking. All rights reserved.</p>
                    <p>For support: ${process.env.EMAIL_USER} | +1 555 010 9999</p>
                </div>
            </div>
        </body>
        </html>
